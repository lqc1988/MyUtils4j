...
import com.lqc.util.AESUtil;
import com.lqc.util.ConstUtil;
import com.lqc.util.HttpUtils;
import com.lqc.util.RandomStringUtils;
import com.lqc.vo.weixin.WXMiniPhoneVO;
import com.lqc.vo.weixin.WXMiniSessionVO;
import com.lqc.vo.weixin.WXMiniUserInfoVO;
import com.lqc.vo.weixin.WXSignatureVO;
...
public class WxUtil {
    ...
    public String getAccessToken(String appId, String secret) throws Exception {
        ...
        if (StringUtils.isNotBlank(resp)) {
            JSONObject jso = JSONObject.parseObject(resp);
            if (null != jso.get("errcode") && jso.getInteger("errcode") != 0) {
                logger.error(opt + "，失败：" + jso.getString("errmsg"), jso.getInteger("errcode"));
                throw new MyException(jso.getString("errmsg"), jso.getInteger("errcode"));
            }
            result = jso.getString("access_token");
            Long expires = (null == jso.get("expires_in")) ? ConstUtil.WX_EXPIRE_SECONDS : jso.getLong("expires_in");
            //cache.set(util.ConstCacheKey.WX_ACCESS_TOKEN_KEY, result, expires);
        } else {
            logger.error(opt + ",失败：响应结果为null");
            throw new MyException(opt + ",失败：响应结果为null");
        }
        //}
        return result;
    }
    ...
    public String getOpenID(String code, String appId, String secret) throws Exception {
        ...
        if (StringUtils.isBlank(code)) {
            ...
            throw new MyException(ResultEnum.NOT_WX.getDisplay(), ResultEnum.NOT_WX.getValue());
        }
        String result;
        StringBuilder url_wx = new StringBuilder();
    ...
    public WXMiniSessionVO getSession_wx_mini(String code, String appId, String secret) throws Exception {
        ...
        if (StringUtils.isNotBlank(resp)) {
            respData = JSONObject.parseObject(resp, WXMiniSessionVO.class);
            if (null != respData.errcode && 0 != respData.errcode) {
                logger.error(opt + "失败：" + respData);
                throw new MyException(respData.errmsg, respData.errcode);
            }
    ...
    public void sendNotice(WxMessage wxMessage, String appId, String secret) throws Exception {
        ...
        if (null == wxMessage) {
            ...
            throw new MyException(ResultEnum.ERR.getDisplay());
        }
        String access_token = getAccessToken(appId, secret);
        StringBuilder url_wx = new StringBuilder();
        url_wx.append("https://api.weixin.qq.com/cgi-bin/message/template/send?");
        url_wx.append("access_token=" + access_token);
        String body = JSONObject.toJSONString(wxMessage);
    ...
    public WXMiniPhoneVO decrypt_phone_wx_mini(String encryptedData, String iv,
        ...
        WXMiniPhoneVO vo = JSONObject.parseObject(plainData, WXMiniPhoneVO.class);
    ...
    public WXMiniUserInfoVO decrypt_UserInfo_wx_mini(String encryptedData, String iv,
        ...
        logger.debug(opt + "开始。");
        String plainData = decrypt_wx_mini(opt, encryptedData, iv, appid, sessionKey);
        WXMiniUserInfoVO vo = JSONObject.parseObject(plainData, WXMiniUserInfoVO.class);
        logger.debug(opt + "成功，结果：" + vo.toString());
        return vo;
    }
