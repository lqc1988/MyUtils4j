 ...
 */
@Component
public class WxMainHelper {
    private final static Logger logger= LoggerFactory.getLogger(util.WxMainHelper.class);
    /**
	 * 配置文件
	 */
	public static Config conf = ConfigFactory.load();
	public static final String wx_appid = conf.getString("wx.main.appid");
	public static final String wx_secret = conf.getString("wx.main.secret");
    /**
     * 获取 access_token
     *
     * @return
     *
     */
    public String getAccessToken(RedisService cache) throws Exception {
        String opt = "获取微信access_token";
        String result;
        if (null != cache.get(ConstUtil.WX_MAIN_ACCESS_TOKEN)) {
            result = cache.get(ConstUtil.WX_MAIN_ACCESS_TOKEN).toString();
...
//            logger.info(opt + ",请求：" + url_sb);
            ...
            if (StringUtils.isNotBlank(resp)) {
                ...
                Long expires= (null==jso.get("expires_in"))?ConstUtil.WX_MAIN_EXPIRE_SECONDS:jso.get("expires_in").asLong();
                cache.set(ConstUtil.WX_MAIN_ACCESS_TOKEN, result, expires);
            } else {
     ...
     *
     */
    public String getJsTicket(RedisService cache) throws Exception {
        ...
        if (null != cache.get(ConstUtil.WX_MAIN_ACCESS_TOKEN)) {
            ...
            String access_token = getAccessToken(cache);
            StringBuilder url_sb = new StringBuilder();
     ...
     *
     */
    public WXVo generateJSSignature(String url,RedisService cache)  {
    	WXVo result = null;
    	try {
    		result = new WXVo();
    		String nonceStr= RandomStringUtils.randomAlphabetic(8);
            String jsapi_ticket = getJsTicket(cache);
